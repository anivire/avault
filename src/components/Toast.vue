<template>
    <div @click="emit('remove')">
        <div
            ...
            class="flex flex-row gap-2 bg-gradient-to-r md:w-96 w-80 rounded-s-lg p-4 transition-all duration-200 ease-in-out cursor-pointer border-2 border-zinc-800 shadow-xl">
            ...
            <div class="flex flex-col">
                <p class="md:text-base text-sm font-bold">{{ title }}</p>
                <p class="md:text-sm text-xs text-zinc-400">{{ description }}</p>
            </div>
            <div class="bg-zinc-800 rounded-sm w-full h-1 absolute left-0 bottom-0">
                <div
